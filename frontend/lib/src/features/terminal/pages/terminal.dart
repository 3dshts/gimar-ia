...
class _TerminalPageState extends State<TerminalPage> {
  ...
  void _registerIFrameView() {
    ui.platformViewRegistry.registerViewFactory(
      _TerminalConfig.viewType,
      (int viewId) {
        final iframe = web.document.createElement('iframe') as web.HTMLIFrameElement;
        
        iframe.src = _TerminalConfig.baseUrl; // Carga la URL limpia
        iframe.style.width = _IFrameStyles.width;
        iframe.style.height = _IFrameStyles.height;
        iframe.style.border = _IFrameStyles.border;
        
        ApiLogger.info('Cargando iframe sin parámetros: ${_TerminalConfig.baseUrl}', 'TerminalPage');
  ...
  Widget build(BuildContext context) {
    return const SizedBox.expand(
      child: HtmlElementView(
        viewType: _TerminalConfig.viewType,
      ),
    );
  }
}